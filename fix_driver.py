...
# 1. Add Icons and PaymentLinkTutorial
target1 = '''export default function DriverPage() {'''
replacement1 = '''const IconProfileColor = () => (
...
# 3. Replace pending screen
target3 = '''        <h2 style={{ fontSize: '1.25rem', fontWeight: 800, margin: 0, textAlign: 'center' }}>Tu cuenta está en revisión</h2>
        ...
        <div style={{ width: '100%', borderTop: '1px solid var(--border)', paddingTop: '20px', marginTop: '10px' }}>
          <h3 style={{ fontSize: '0.9rem', fontWeight: 800, marginBottom: '12px', color: '#fff', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" id="Cash-Payment-Coin-Dollar--Streamline-Ultimate" height="20" width="20" style={{ flexShrink: 0 }}>
              ...
              <path stroke="#191919" strokeLinecap="round" strokeLinejoin="round" d="M15.3477 18.1739v-0.9565" strokeWidth={1}></path>
              <path stroke="#191919" strokeLinecap="round" strokeLinejoin="round" d="m13.4346 6.26087 0.9565 -0.95652 2.8696 2.86955 -0.9565 0.95654" strokeWidth={1}></path>
              <path stroke="#191919" strokeLinecap="round" strokeLinejoin="round" d="M13.4346 15.3478c1.0565 1.0566 2.7694 1.0566 3.8261 0" strokeWidth={1}></path>
              <path stroke="#191919" strokeLinecap="round" strokeLinejoin="round" d="m6.73926 21.0869 11.47824 -6.6956" strokeWidth={1}></path>
            </svg>
            Plan FLEX
          </h3>
          <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '16px', lineHeight: '1.4' }}>
            Únete a la membresía más rentable. 0% de comisión por viaje.
          </p>
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--accent)', marginBottom: '4px' }}>
                {formatCLP(10000)} <span style={{ fontSize: '0.75rem', fontWeight: 400, color: 'var(--text-muted)' }}>/ 10 días</span>
              </div>
              <ul style={{ paddingLeft: '20px', margin: 0, fontSize: '0.8rem', color: 'var(--text-secondary)' }}>
                <li>Viajes ilimitados</li>
                <li>Soporte preferencial</li>
                <li>Acceso a zonas de alta demanda</li>
              </ul>
            </div>
          </div>
          
          <div style={{ marginTop: '20px' }}>
            {driver.taxPendingReview ? (
              <div style={{ background: 'rgba(255,193,7,0.1)', border: '1px solid var(--warning)', borderRadius: 'var(--radius)', padding: '16px', textAlign: 'center' }}>
                <p style={{ margin: 0, fontSize: '0.85rem', color: 'var(--warning)', fontWeight: 700 }}>
                  Tus documentos de inicio de actividades están en revisión.
                </p>
                <p style={{ margin: '8px 0 0', fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                  Te notificaremos una vez que hayan sido validados.
                </p>
              </div>
            ) : !driver.taxCompliant ? (
              <div style={{ background: 'rgba(255, 69, 96, 0.05)', padding: '16px', borderRadius: 'var(--radius)', border: '1px solid var(--danger)', marginBottom: '16px' }}>
                <p style={{ fontSize: '0.85rem', color: 'var(--danger)', margin: '0 0 12px', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                  Aviso Importante (SII)
                </p>
                <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', margin: '0 0 12px', lineHeight: '1.4' }}>
                  Para activar tu cuenta es requisito obligatorio subir tu <strong>Inicio de Actividades</strong> o carpeta tributaria.
                </p>
                <input
                  type="file"
                  id="taxDocPending"
                ...
                <button 
                  ...
                  style={{ background: 'var(--danger)', color: '#fff', fontSize: '0.85rem' }}
                  onClick={() => document.getElementById('taxDocPending')?.click()}
                  disabled={uploadingReceipt}
                >
                  {uploadingReceipt ? 'Subiendo...' : 'Subir Documento (SII)'}
                </button>
              </div>
            ) : (
              <button 
...
replacement3 = '''        <h2 style={{ fontSize: '1.25rem', fontWeight: 800, margin: 0, textAlign: 'center' }}>Tu cuenta está en revisión</h2>
        <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', margin: 0, textAlign: 'center', lineHeight: '1.5' }}>
...
# 4. Modify Dashboard "Membresía" block
target4 = '''          {driver.isPromoActive && (
            <div style={{ background: 'rgba(212,175,55,0.1)', border: '1px solid #D4AF37', borderRadius: 'var(--radius)', padding: '12px', marginBottom: '20px', display: 'flex', gap: '12px', alignItems: 'center' }}>
              <div style={{ background: 'rgba(212,175,55,0.2)', padding: '8px', borderRadius: '50%' }}>
                <IconShieldLock size={20} />
              </div>
              <div>
                <div style={{ color: '#D4AF37', fontWeight: 800, fontSize: '0.9rem' }}>¡Promoción Activa!</div>
                <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.75rem', lineHeight: '1.4', marginTop: '4px' }}>
                  Puedes conectarte y recibir viajes.
                </div>
              </div>
            </div>
          )}

          {!driver.membershipPaid && !driver.isPromoActive && (
            <div style={{ background: 'rgba(255,69,96,0.05)', border: '1px solid var(--danger)', borderRadius: 'var(--radius)', padding: '16px', marginBottom: '20px' }}>
              <h3 style={{ fontSize: '0.95rem', fontWeight: 800, margin: '0 0 12px 0', color: '#fff' }}>
                Plan {driver.membershipPlan}
              </h3>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '16px' }}>
                Tu membresía no está activa. Para poder conectarte y recibir viajes, necesitas pagar tu cuota.
              </p>
              {driver.membershipPlan === 'FLEX' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Monto a pagar:</span>
                    <span style={{ fontSize: '1rem', fontWeight: 800, color: 'var(--accent)' }}>{formatCLP(10000)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Vigencia:</span>
                    <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#fff' }}>10 días</span>
                  </div>
                </div>
              )}
              {driver.membershipPlan === 'BLACK' && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Monto a pagar:</span>
                    <span style={{ fontSize: '1rem', fontWeight: 800, color: 'var(--accent)' }}>{formatCLP(60000)}</span>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>Vigencia:</span>
                    <span style={{ fontSize: '0.85rem', fontWeight: 700, color: '#fff' }}>30 días</span>
                  </div>
                </div>
              )}
              ...
              {driver.taxPendingReview ? (
                ...
                <div style={{ background: 'rgba(255, 69, 96, 0.05)', padding: '16px', borderRadius: 'var(--radius)', border: '1px solid var(--danger)', marginBottom: '16px' }}>
                  ...
                  <p style={{ fontSize: '0.8rem', color: 'var(--text-secondary)', margin: '0 0 12px', lineHeight: '1.4' }}>
                    Para poder pagar tu membresía y conectarte, es requisito obligatorio subir tu <strong>Inicio de Actividades</strong> o carpeta tributaria.
                  </p>
                  <input
                    type="file"
                    id="taxDoc"
...
replacement4 = '''          {driver.isPromoActive && (
            <div style={{ 
              ...
              <div style={{ background: 'rgba(0,0,0,0.3)', padding: '16px', borderRadius: '12px', marginBottom: '16px' }}>
                <div style={{ color: 'var(--accent)', fontSize: '1.25rem', fontWeight: 900, marginBottom: '4px' }}>Free PASS 10 días</div>
                <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Te quedan <strong style={{ color: '#fff' }}>10 días</strong></div>
              </div>
