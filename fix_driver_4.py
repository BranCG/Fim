...
if 'const IconProfileColor = () => (' not in c:
    c = c.replace('export default function DriverPage() {', icons)

# Add state
state_search = 'const [passengerConfirmed, setPassengerConfirmed] = useState(false);'
if 'const [showMpTutorial, setShowMpTutorial] = useState(false);' not in c:
    c = c.replace(state_search, state_search + '\n  const [showMpTutorial, setShowMpTutorial] = useState(false);')

# Now the pending block.
pending_block_start = "if (driver.status === 'pending') return ("
pending_block_end = "      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', width: '100%', maxWidth: '440px' }}>"
pending_replacement = '''if (driver.status === 'pending') return (
...
if 'Una vez que se validen los documentos' not in c:
    idx_start = c.find(pending_block_start)
    if idx_start != -1:
        idx_end = c.find(pending_block_end, idx_start)
        if idx_end != -1:
            c = c[:idx_start] + pending_replacement + c[idx_end:]

# Now the dashboard promo logic (gold card)
promo_block_start = "{driver.isPromoActive && ("
promo_block_end_marker = "            </div>\n          )}"
promo_replacement = '''{driver.isPromoActive && (
            <div style={{ 
...
if '🎉' not in c:
    idx_pstart = c.find(promo_block_start)
    if idx_pstart != -1:
        # Find the end of the block... since it's `</div>\n          )}`, we look for `)}\n`
        # But maybe we just regex it
        match = re.search(r'\{driver\.isPromoActive && \([\s\S]*?\n\s*\)\}', c[idx_pstart:])
        if match:
            c = c[:idx_pstart] + promo_replacement + c[idx_pstart + match.end():]

# Now inject Mercado Pago panel before "Ponerse en línea"
# We find: 
...
# There is exactly one place where the bottom action buttons are in the Dashboard.
# <div style={{ display: 'flex', gap: '10px' }}>
#   <button className={`btn ${isOnline ? 'btn-danger' : 'btn-accent'} btn-block btn-lg`}
if 'PaymentLinkTutorial showMpTutorial' not in c and '¡Vincular Mercado Pago Connect!' not in c:
    c = c.replace("<div style={{ display: 'flex', gap: '10px' }}>\n            <button className={`btn ${isOnline ? 'btn-danger' : 'btn-accent'} btn-block btn-lg`}", 
                  mp_panel + "\n            <button className={`btn ${isOnline ? 'btn-danger' : 'btn-accent'} btn-block btn-lg`}")
