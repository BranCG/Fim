  ...
  const handleSendChat = async () => {
    ...
    try {
      ...
      if (data.choices && data.choices[0] && data.choices[0].message) {
        setChatMessages([...newHistory, data.choices[0].message]);
      } else {
        setChatMessages([...newHistory, { role: 'assistant', content: '❌ Error: No se pudo obtener respuesta del agente.' }]);
      }
    } catch (e) {
      setChatMessages([...newHistory, { role: 'assistant', content: '❌ Error de conexión con el servidor.' }]);
    } finally {
    ...
    <div style={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <main style={{ marginLeft: '220px', flex: 1, padding: '28px', minHeight: '100vh' }}>
        ...
        <div style={{ position: 'fixed', bottom: '30px', right: '30px', zIndex: 9999, display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
          ...
          {/* Chat Window */}
          {isChatOpen && (
            <div className="card animate-slide-up" style={{ width: '380px', height: '500px', marginBottom: '16px', padding: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', border: '1px solid var(--accent)', boxShadow: '0 10px 40px rgba(0,0,0,0.5)' }}>
              
              {/* Header */}
              <div style={{ background: 'var(--bg-secondary)', padding: '16px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <div style={{ background: 'var(--accent)', borderRadius: '50%', width: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#000' }}>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 12a10 10 0 1 0 20 0 10 10 0 1 0-20 0" /><path d="M12 2a10 10 0 0 1 10 10" /><path d="M12 22a10 10 0 0 0 10-10" /></svg>
                  </div>
                  <div>
                    <h3 style={{ fontSize: '0.95rem', margin: 0 }}>Fim AI Copilot</h3>
                    <div style={{ fontSize: '0.7rem', color: 'var(--success)', display: 'flex', alignItems: 'center', gap: '4px' }}>
                      <span style={{ width: '6px', height: '6px', background: 'var(--success)', borderRadius: '50%' }}></span> En línea
                    </div>
                  </div>
                </div>
                <button onClick={() => setIsChatOpen(false)} style={{ background: 'transparent', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}>
                  ✕
                </button>
              </div>
              ...
              <div style={{ flex: 1, padding: '16px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {chatMessages.length === 0 ? (
                  <div style={{ textAlign: 'center', color: 'var(--text-muted)', marginTop: '40px', fontSize: '0.85rem' }}>
                    ...
                    Soy tu director de estrategia.<br/>Pregúntame sobre el mercado o métricas.
                  </div>
                ) : (
                ...
                {isChatLoading && (
                  <div style={{ alignSelf: 'flex-start', background: 'var(--bg-secondary)', padding: '10px 14px', borderRadius: '12px', fontSize: '0.85rem', color: 'var(--text-muted)' }}>
                    Pensando...
                  </div>
                )}
              </div>
              ...
              <div style={{ padding: '12px', borderTop: '1px solid var(--border)', background: 'var(--bg-card)' }}>
                <form onSubmit={(e) => { e.preventDefault(); handleSendChat(); }} style={{ display: 'flex', gap: '8px' }}>
                  <input
                    ...
                    style={{ flex: 1, background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: 'var(--radius-full)', padding: '8px 16px', color: '#fff', fontSize: '0.85rem', outline: 'none' }}
                  />
                  <button type="submit" disabled={isChatLoading || !chatInput.trim()} style={{ background: 'var(--accent)', color: '#000', border: 'none', borderRadius: '50%', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', opacity: (isChatLoading || !chatInput.trim()) ? 0.5 : 1 }}>
          ...
          <button 
            ...
            style={{
              width: '60px', height: '60px', borderRadius: '50%', background: 'var(--accent)', color: '#000',
              border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
              boxShadow: '0 4px 14px rgba(212, 175, 55, 0.4)', transition: 'transform 0.2s ease',
